...
class SSD_Detector:
    ...
    def preprocess(self, src_img):
        ...
        # input_net_img = input_net_img.transpose(2, 0, 1)  # 将图像的轴顺序从 (height, width, channels) 转换为 (channels, height, width)
         # Save the preprocessed image as a binary file

        flattened_rgb_data = input_net_img.copy().flatten()
        flattened_rgb_data = np.array(flattened_rgb_data).astype(np.uint8)
        output_path        = './input_net_img_onnx_no_sig_8_1.bin'
    ...
    def inference(self, src_img):
        ...
        boxes, confidences   = self.sess.run(None, {input_name: input_net_img})

        
        
        # # --------------------------------  float32 model use -------------------------------
        # confidences_path = './confidences.bin'
        # boxes_path = './boxes.bin'
        # confidences.flatten().astype(np.float32).tofile(confidences_path)
        # boxes.flatten().astype(np.float32).tofile(boxes_path)

        # --------------------------------  int8 model use ----------------------------------
        confidences_path = './confidences_st.bin'
        boxes_path = './boxes_st.bin'
        confidences.flatten().astype(np.int8).tofile(confidences_path)
        boxes.flatten().astype(np.int8).tofile(boxes_path)

        for i in range(5):
            print(boxes[0][i])
        print("\n \n")
        for i in range(10):
            print(confidences[0][i])
        confidences = self.softmax(confidences[0], axis=1)
        print("\n \n")
        for i in range(10):
            print(confidences[i])
...
if __name__ == "__main__":
    ...
    input_width        = 192
    box_conf           = 0.9
    input_img_path     = "./21_1692932849057.jpg"
    
    
    onnx_path          = "./model_without_sig_HWC_QDQ_quant.onnx"   # output float32 model

    # onnx_path        = "./model_without_sig_HWC_QDQ_quant_OE_2_3_2.onnx"  # output int8 model
